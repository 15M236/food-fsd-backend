...
const {hashPassword,hashCompare,createToken,decodeToken,validateToken,adminGaurd,deliveryCheck} = require('../config/auth')
...
router.get('/get-orders/:email',validateToken,async(req,res)=>{
  try {
    let orders = await orderModel.find({orderedBy:req.params.email})
    res.send({
      statusCode:200,
      orders
    })
...
// router.get('/get-orders', validateToken , deliveryCheck , async(req,res)=>{
//  try {
